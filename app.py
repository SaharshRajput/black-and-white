...
def make_grayscale(input_image):
	...
	decode_array_img=cv2.imdecode(image_array,cv2.IMREAD_UNCHANGED)
	print("Decoded array image:",decode_array_img)

	converted_img=cv2.cvtColor(decode_array_img,cv2.COLOR_RGB2GRAY)
	status, output=cv2.imencode('.PNG',converted_img)
	print('status:',status)
